    ...
    async function fetchTxs() {
      ...
      try {
        const params = new URLSearchParams({
          page: String(txsPage),
          pageSize: String(txsPageSize),
        });
        const res = await fetch(
          `/api/admin/users/${id}/transactions?${params.toString()}`
        );
    ...
    async function fetchActivity() {
      ...
      try {
        ...
        items.forEach((tx: any) => {
          ...
          if (!map.has(dateKey))
            map.set(dateKey, { date: dateKey, deposits: 0, withdrawals: 0 });
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <button
            onClick={() => toggleBlock(!user.isBlocked)}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card>
          ...
          <CardContent>
            ...
            <p>
              ...
              {Number(user.inrBalance ?? 0).toLocaleString()}
            </p>
        ...
        <Card className="md:col-span-2">
          ...
          <CardContent>
            {Array.isArray(user.payoutProfiles) &&
            user.payoutProfiles.length > 0 ? (
              <div className="space-y-3">
                {user.payoutProfiles.map((p: any) => (
                  <div key={p.id} className="border border-border rounded p-3">
                    <div className="flex justify-between">
                      <div>
                        <p className="font-medium">
                          {p.type} {p.isActive ? "(Active)" : "(Inactive)"}
                        </p>
                    ...
                    {p.type === "UPI" ? (
                      <p className="mt-2">
                        <strong>VPA:</strong> {p.upiVpa ?? "—"}
                      </p>
                    ) : (
                      <>
                        <p className="mt-2">
                          <strong>Account:</strong> {p.accountNumber ?? "—"}
                        </p>
                        <p className="text-sm">
                          <strong>IFSC:</strong> {p.ifsc ?? "—"}
                        </p>
                        <p className="text-sm">
              ...
              <div className="text-sm text-muted-foreground">
                No payout profiles found for this user.
              </div>
            )}
          </CardContent>
        </Card>
                    ...
                    {txs.map((t) => (
                      <tr
                        ...
                        <td className="py-3 px-2 font-semibold">
                          ₹{Number(t.inrAmount ?? 0).toLocaleString()}
                        </td>
                        <td className="py-3 px-2">
                          <span
